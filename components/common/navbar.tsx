...
function NavBar() {
  ...
  return (
    <div className="md:sticky md:top-0   md:shadow-none z-20 mt-20 md:mt-0">
      {/* DESKTOP */}
      <div className=" hidden lg:block animate-in fade-in zoom-in bg-white p-4">
        <div className="flex justify-between mx-4 items-center">
          ...
          <div className="flex gap-5 xl:gap-12.5 text-[16px] items-center select-none">
            {links.map((item, index) => (
              <div key={index} className="flex gap-2">
                <p
                  className={`hover:text-primary cursor-pointer flex items-center gap-2  font-medium text-gray`}
                >
                  {item.name}
                </p>
                {item.badgeCount ? (
      ...
      >
        <div className="flex justify-between mx-2.5">
          <div className="flex gap-12.5 text-[16px] items-center select-none">
            <img src="/images/logo.svg" alt="logo" />
          </div>
